...
from definitions import (
    Age, Backstory, BackstoryCategory,
    GenderAlign, GenderKits, Location,
    Pronouns, Rank, RedSkill,
    DEFAULT_PRONOUNS,
    cast_to_backstory, cast_to_backstory_category, cast_to_gender_align, cast_to_gender_kits,
    cast_to_language, cast_to_location, cast_to_rank, parse_to_pronouns, cast_to_skill, SkillCategory,
    ConditionCategory,
)
from scripts._red.cat_tracker import cat_tracker
from scripts._red.events.red_history import RedHistory
from scripts._red.cats.red_name import RedName
from scripts._red.cats.red_pelt import CatPelt
from scripts._red.cats.condition import Condition, get_random_permanent_condition
...
class RedCat:
    ...
    TODO
     - create_cat (include changes to CatTracker)
     - kill_cat (include changes to CatTracker)
    """
    ...
    # status
    location: Location
    clan_prefix: str = None
    health: CatHealth = CatHealth()
    rank: Rank = None
    backstory: Backstory = Backstory.Unknown
    moons: int
    skills: dict
    ...
    # TODO there shouldn't be both moons and age
    age: Age

    mate_ids: list[int] = [-1]
    current_apps: list[int] | bool = False
    prev_apps: list[int] | bool = False
    current_mentors: list[int] | bool = False
    prev_mentors: list[int] | bool = False
    ...
    def __init__(self, save_file: dict = None, **kwargs, ):
        ...
        if save_file:
            self._parse_save_file(save_file)
            return
        else:
            if ( "backstory" in kwargs.keys() or "backstory_category" in kwargs.keys() ) and \
                    "moons" in kwargs.keys() and "clan_name" in kwargs.keys():
                self.create_cat(**kwargs)
            else:
                backstory = "missing"
                ...
                raise KeyError(f"Creating a cat from scratch requires the creator to "
                               ...
                               f"Backstory (/category): {backstory}\n"
        ...
        if kwargs:
            logger.warning(f"Did not use the following passed arguments when "
                           f"generating RedCat object with id {self.id}: {kwargs}")
        return

    def _parse_save_file(self, save: dict):
        """ """
        ...
        if "gender" in save_file_sections:
            ...
            if save["gender"]["custom_pronouns"]:
                self.pronouns = parse_to_pronouns(
                    save["gender"][cast_to_language(conf.settings.Language)]["custom_pronouns"]
                )
            else:
                self.pronouns = DEFAULT_PRONOUNS[cast_to_language(conf.settings.Language)][self.gender_align]
        else:
            raise KeyError(f"Could not find key 'gender' in save file cats.yaml for RedCat {self.id}")
        ...
        if "name" in save_file_sections:
            ...
            raise AttributeError(f"Could not assign a name to RedCat {self.id} from the save file cats.yaml")

        # parse the cat's status
        if "status" in save_file_sections:
            save_file_sections.remove("status")
            self.clan_prefix = save["status"]["clan_prefix"]
            self.location = cast_to_location(save["status"]["location"])
            self.health = save["status"]["health"]
            self.rank = cast_to_rank(save["status"]["rank"])
        else:
            raise KeyError(f"Could not find key 'status' in save file cats.yaml for RedCat {self.id}")
        ...
        if "pelt" in save_file_sections:
            ...
            raise KeyError(f"Could not find key 'pelt' in save file cats.yaml for RedCat {self.id}")
        ...
        if "flags" in save_file_sections:
            ...
            raise KeyError(f"Could not find key 'flags' in save file cats.yaml for RedCat {self.id}")

        # if save_file_sections still has any keys in it, then there was a section we weren't looking for
        if save_file_sections:
            raise KeyError(f"Unexpected key or keys in cats.yaml for RedCat {self.id}: {save_file_sections}")
        ...
        if "personality" in save_file_sections:
            ...
            raise KeyError(f"Could not find key 'personality' in save file cats.yaml for RedCat {self.id}")

        # TODO parse the cat's history
        if "history" in save_file_sections:
            raise NotImplementedError(f"RedCat objects can't parse a 'history' section from cats.yaml yet")
        else:
            raise KeyError(f"Could not find key 'history' in save file cats.yaml for RedCat {self.id}")
        ...
        if "relationships" in save_file_sections:
            ...
            raise KeyError(f"Could not find key 'relationships' in save file cats.yaml for RedCat {self.id}")
        ...
        if "skills" in save_file_sections:
            ...
            raise KeyError(f"Could not find key 'skills' in save file cats.yaml for RedCat {self.id}")

    # -------------------------------- Birth and death ------------------------------- #

    def create_cat(self, clan_name: str, moons: int, parents: list = None, **kwargs):
        """ Generate a cat.
        ...
        def get_social_skill_xp() -> int:
            ...
            for _ in range(self.moons):
                xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_social)
            return xp
        ...
        def get_rank_skill_xp() -> int:
            ...
            if self.age in (Age.Newborn, Age.Kitten):
                ...
                moons_spent_learning: int = (self.moons - conf.cat_config.apprentice_age_moons)
                for moon in range(moons_spent_learning):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[self.rank])
                ...
                moons_spent_learning = conf.cat_config.warrior_age_moons - conf.cat_config.apprentice_age_moons
                for _ in range(moons_spent_learning):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.WarriorApp])
                moons_spent_working = conf.cat_config.elder_age_moons - conf.cat_config.warrior_age_moons
                for _ in range(moons_spent_working):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.Warrior])

            elif self.rank is Rank.Healer:
                # healers have different learning rates than warriors and mediators
                moons_spent_working = self.moons - conf.cat_config.warrior_age_moons
                for _ in range(moons_spent_working):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.Healer])
                moons_spent_learning = conf.cat_config.warrior_age_moons - conf.cat_config.apprentice_age_moons
                for _ in range(moons_spent_learning):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.WarriorApp])
                ...
                moons_spent_working = self.moons - conf.cat_config.warrior_age_moons
                for _ in range(moons_spent_working):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.Mediator])
                moons_spent_learning = conf.cat_config.warrior_age_moons - conf.cat_config.apprentice_age_moons
                for _ in range(moons_spent_learning):
                    xp += random.randint(*conf.cat_config.base_timeskip_xp_gain_rank[Rank.MediatorApp])
        ...
        if "backstory_category" in kwargs.keys():
            if self.backstory is Backstory.Unknown:
                bc: BackstoryCategory = cast_to_backstory_category(kwargs["backstory_category"])
                self.backstory = random.choice( [b for b in list(Backstory) if b.category is bc] )
        ...
        if "gender_kits" in kwargs:
            ...
            if one_in_num_chance(num=conf.cat_config.chance_gender_kits_intersex):
        ...
        if "rank" in kwargs.keys():
            ...
            kwargs.pop("rank")
        if self.backstory.category is BackstoryCategory.Kittypet:
            self.rank = Rank.Kittypet
            self.location = Location.TwolegNest
        if self.backstory.category is BackstoryCategory.Loner:
            self.rank = Rank.Loner
            self.location = Location.Wilderness
        if self.backstory.category is BackstoryCategory.Rogue:
            self.rank = Rank.Rogue
            self.location = Location.Wilderness
        if self.moons < conf.cat_config.apprentice_age_moons:
            self.rank = Rank.Kit
            self.location = Location.ClanNursery
        elif self.moons <= conf.cat_config.warrior_age_moons:
            self.rank = Rank.WarriorApp
            self.location = Location.ClanApprentice
        elif self.moons >= conf.cat_config.elder_age_moons and one_in_num_chance(3):
        ...
        if "skills" in kwargs:
            ...
            if one_in_num_chance(conf.cat_config.chance_clairvoyant):
        ...
        if one_in_num_chance(conf.cat_config.chance_condition):
            pc = []
            for parent in parents:
                pc.extend( [c.name for c in parent.health.conditions if c.category is ConditionCategory.Permanent] )
            c: Condition = get_random_permanent_condition(parent_conditions=pc)
            if c.name in pc:
                c.is_genetic = True
        ...
        clan_obj = cat_tracker.all_clans[self.clan_prefix]
        if clan_obj.camp.value:
            biome = clan_obj.camp.biome
        self.name = RedName(prefix=prefix, suffix=suffix, biome=biome)

        cat_tracker.add_new_cat(cat_obj=self, clan_name=self.clan_prefix, alive=True)
        return
    ...
    def remove_condition(self, condition: Condition):
        ...
        if condition in self.health.conditions:
            ...
            logger.warning(f"Was asked to remove a condition from cat {self.id} that it didn't have")
        return
