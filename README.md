...
ClanSimulator (aka ClanSim) is an overhaul of the warrior cat game ClanGenerator (aka ClanGen) built using Python and 
Pygame.

### Description
The main idea behind this overhaul is to increase the difficulty of the game after the point where you have enough cats 
to ensure survival. This is accomplished in two main ways:

1) **Raise the stakes** in the form of new kinds of consequences for negative event outcomes, beyond cats being hurt 
and dying.
2) **Increase gameplay complexity** by overhauling patrols and the relationship system, between cats in the same Clan 
and between different Clans.
...
- The hunting and fighting skills have been broken up into biome-specific variants. 
...
In order to introduce more consequential results outside cats dying, there is now a tiled game map where each square is 
either claimed by a specific Clan or is explicitly neutral area (e.g. for communicating with StarClan and/or holding 
Gatherings). Your Clan needs to regularly patrol their territory (especially squares that are Clan borders!).
...
- Each tile belongs to a biome, which can influence which events can happen in it and which skills cats need to 
...
| 2    | Name the Clan       | Name the Clan and choose icon                          |
| 3    | Pick leader         | Pick or build leader                                   |
| 4    | Pick deputy         | Pick or build deputy, or leave choice up to the leader |
| 5    | Pick medicine cat   | Pick or build medicine cat                             |
| 6    | Pick 4-7 other cats | Pick or build 4-7 other cats                           |
| 7    | Choose Clan camp    | Choose Clan camp                                       |
| 8    | Choose Clan icon    |                                                        |
...
### [ClanGen Discord Server](https://discord.gg/clangen) || [ClanGen Official website](https://clangen.io) || [ClanGen Itch.io Page](https://sablesteel.itch.io/clan-gen-fan-edit) 
